...
class App extends Component {
   ...
   async componentDidMount() {
      this.getNotes();
      this.createNoteListener = API.graphql(graphqlOperation(onCreateNote, { owner: (await Auth.currentUserInfo()).username })).subscribe({
         next: noteData => {
      ...
      this.deleteNoteListener = API.graphql(graphqlOperation(onDeleteNote, { owner: (await Auth.currentUserInfo()).username })).subscribe({
         next: noteData => {
      ...
      this.updateNoteListener = API.graphql(graphqlOperation(onUpdateNote, { owner: (await Auth.currentUserInfo()).username })).subscribe({
         next: noteData => {
   ...
   render() {
      ...
      return (
         <div className="flex flex-column items-center justify-center pa3 bg-washed-red">
            <h1 className="code f2-l">NOTE</h1>
            {/* Note Form */}
